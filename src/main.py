...
class VerticalFarming:
    ...
    def captureAndsendImages(self) -> None:
        return self.cam.capture_images() #TODO 
    
    def schedule_capture(self, interval) -> None:
        _thread.start_new_thread(self.capture_thread, (interval,))
        self.captureAndsendImages()

    def capture_thread(self, interval) -> None:
        while True:
            currentTime = time.time() # Gets current time
            nextcaptureTime = currentTime + interval # Calculates time until next capture
            image_path = self.captureAndsendImages # Captures and sends images
